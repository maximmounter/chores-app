...
function launchConfetti() {
  const colours = ["#2563eb", "#60a5fa", "#fbbf24", "#34d399", "#f472b6", "#fb923c", "#e879f9", "#ffffff"];
  ...
  for (let i = 0; i < 150; i++) {
    ...
    let rot  = (Math.random() - 0.5) * 900;
    let dur  = 700 + Math.random() * 600;

    requestAnimationFrame(() => requestAnimationFrame(() => {
      el.style.transition = "left " + dur + "ms cubic-bezier(0.05,0.9,0.3,1)," +
                            "top "  + dur + "ms cubic-bezier(0.05,0.9,0.3,1)," +
                            "transform " + dur + "ms ease-out," +
                            "opacity 200ms ease-in " + (dur * 0.7) + "ms";
      el.style.left      = endX + "px";
      el.style.top       = endY + "px";
      el.style.transform = "rotate(" + rot + "deg) scale(0.2)";
      el.style.opacity   = "0";
    }));
